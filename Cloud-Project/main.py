...
def fillM(alist):
    ...
    for index in range(len(tlist2)):
        m.append(int((-(tlist2[index]) * np.log(pvalue) / (pow(np.log(2), 2))) + 1))
    listm = list(m)
    return listm
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: Cloud-Computing project <input file> [<output file>]", file=sys.stderr)
        sys.exit(-1)
    ...
    list_m = fillM(valuesM.collect())

    # Fase MAPP
    finalRDD = keyValueRDD.mapPartitions(mapper, preservesPartitioning=True).reduceByKey(lambda x, y: orFilter(x, y), numPartitions=1).saveAsTextFile(
        sys.argv[2])
